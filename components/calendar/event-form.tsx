...
import { useCreateEvent, useDeleteEvent, useUpdateEvent } from "@/services/mutaions";
...
const {mutate:createEvent,isPending:createStatus} =useCreateEvent(handleReset)
const {mutate:updateEvent,isPending:updateStatus} =useUpdateEvent(handleReset)
const {mutate:deleteEvent,isPending:deleteStatus} =useDeleteEvent(handleReset)
...
const [alertOpen,setAlertOpen]=useState(false)
  ...
  return (
    <>
    {(id && alertOpen) &&<AlertDialogBox id={id} open={alertOpen} setOpen={setAlertOpen}/>}
    <Dialog onOpenChange={handleClose} open={open}>
      <DialogContent className="sm:max-w-[425px] md:max-w-[650px]">
        <DialogHeader>
          ...
          {id &&( <div className="absolute right-10 top-8">
           <Button variant={"ghost"} color="red" onClick={()=>{setAlertOpen(pre=>!pre)}}>
